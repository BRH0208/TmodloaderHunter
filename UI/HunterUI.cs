...
namespace Hunter.UI
{
	internal class HunterUI : UIState
	{
		...
		public override void Update(GameTime gameTime) {
			...
			if (Main.LocalPlayer.talkNPC == -1 || Main.npc[Main.LocalPlayer.talkNPC].type != NPCType<HunterNPC>()) {
				...
				GetInstance<Hunter>().HunterUserInterface.SetState(null);
			}
		}
				...
				if (hoveringOverReforgeButton) {
					...
					if (Main.mouseLeftRelease && Main.mouseLeft && Main.LocalPlayer.CanBuyItem(upgradeCost, -1) && ItemLoader.PreReforge(_vanillaItemSlot.Item)) {
						...
						switch (Main.rand.Next(13)) { //This case swich could be transformed into an array quite easily
							case 0:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("SlimeSlaying"));
								break;
							case 1:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("UndeadSlaying"));
								break;
							case 2:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("SkySlaying"));
								break;
							case 3:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("WaterSlaying"));
								break;
							case 4:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("HellSlaying"));
								break;
							case 5:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("CorruptionSlaying"));
								break;
							case 6:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("CrimsonSlaying"));
								break;
							case 7:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("BloodSlaying"));
								break;
							case 8:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("SnowSlaying"));
								break;
							case 9:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("PirateSlaying"));
								break;
							case 10:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("JungleSlaying"));
								break;
							case 11:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("HollowSlaying"));
								break;
							default:
								reforgeItem.Prefix(GetInstance<Hunter>().PrefixType("SolarEclipseSlaying"));
								break;
